...
//#######################################################################
FILE_TRACK_C::FILE_TRACK_C (void)
    {
    reset();
    }
...
bool FILE_TRACK_C::getNextEvent (FILE_MIDI_C& mf, uint16_t tickCount)
    {
    uint32_t deltaT;
...
void FILE_TRACK_C::parseEvent (FILE_MIDI_C& mf)
    {
    uint8_t eType;
    uint32_t mLen;
        ...
        case 0x00 ... 0x7f: // MIDI run on message
            {
                ...
                for ( byte z = 2;  z < _mev.size;  z++ )
                    {
                    _mev.data[z] = mf._fd.read ();  // next byte
                    }

                DUMP ("[MID+] Ch: ", _mev.channel);
                DUMPS (" Data:");
                for ( byte z = 0;  z < _mev.size;  z++ )
                    DUMPX (" ", _mev.data[z]);
        ...
        case 0xf7:  // sysex_event = 0xF7 + <len:1> + <data_bytes> + 0xF7
            {
                ...
                for ( byte z = index;  z < minLen;  ++z )
                    sev.data[z] = mf._fd.read ();
                if ( sev.size > minLen )
                    mf._fd.seek (sev.size - minLen);

#if DUMP_DATA
                DUMPS ("[SYSX] Data:");
                for ( uint16_t z = 0;  z < minLen;  z++ )
                    DUMPX (" ", sev.data[z]);
                if ( sev.size > minLen )
                    DUMPS ("...");
#else
        ...
        case 0xff:  // meta_event = 0xFF + <meta_type:1> + <length:v> + <event_data_bytes>
            {
                ...
                switch ( eType )
                    ...
                    case 0x51:  // set Tempo - really the microseconds per tick
                        {
                        uint32_t value = readMultiByte (mf._fd, 3);
                    ...
                    case 0x00:  // Sequence Number
                        {
                        uint16_t x = readMultiByte (mf._fd, 2);
                    ...
                    case 0x20:  // Channel Prefix
                        mev.data[0] = readMultiByte (mf._fd, 1);
...
int FILE_TRACK_C::load (uint8_t trackId, FILE_MIDI_C& mf)
    {
    uint32_t  dat32;
    ...
    // track_chunk = "MTrk" + <length:4> + <track_event> [+ <track_event> ...]
        ...
        mf._fd.read ((byte*)h, MTRK_HDR_SIZE + 1);
        h[MTRK_HDR_SIZE] = '\0';
